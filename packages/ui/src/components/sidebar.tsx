...
import { Separator } from "@cafebot/ui/components/separator";
import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
} from "@cafebot/ui/components/sheet";
import { Skeleton } from "@cafebot/ui/components/skeleton";
...
const SIDEBAR_RESIZE_MIN_WIDTH = 12;
const SIDEBAR_RESIZE_MAX_WIDTH = 28;
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  const { toggleSidebar, setOpen, state, setSidebarWidth } = useSidebar();
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear touch-none select-none group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-none border-2 border-transparent p-2 text-left text-xs font-semibold tracking-[0.12em] uppercase ring-sidebar-ring outline-hidden transition-[width,height,padding,color,background-color,border-color,box-shadow,translate] duration-150 group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:border-sidebar-border hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:shadow-sm active:translate-y-px active:shadow-none focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:border-foreground data-active:bg-primary data-active:text-primary-foreground data-active:shadow-sm data-active:hover:border-foreground data-active:hover:bg-primary data-active:hover:text-primary-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
        default: "",
        outline:
          "border-border bg-background shadow-sm hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:shadow-sm",
      },
      size: {
        default: "h-9 text-xs",
        sm: "h-7 text-[0.65rem]",
